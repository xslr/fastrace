    ...
    : QAbstractTableModel(parent)
{
    ...
    connect(this, &MessageTableModel::chunkDecodeRequested, this, [this](size_t chunkIndex) {
        ...
        QtConcurrent::run([this, analyzer, chunkIndex]() {
            ...
            // Convert to QVector to safely pass across thread boundaries via signals
            QVector<fastrace::TraceMessage> qmsgs(msgs.begin(), msgs.end());
            
            // Invoke slot on the main GUI thread
            QMetaObject::invokeMethod(this, "onChunkDecoded", Qt::QueuedConnection,
                                      Q_ARG(size_t, chunkIndex),
...
{
    if (parent.isValid() || !m_analyzer) return 0;
...
{
    if (!index.isValid() || !m_analyzer) return QVariant();
    ...
    if (role == Qt::DisplayRole) {
        switch (index.column()) {
            ...
            case 6: return QString::number(msg.dataLen);
            case 7: return QString("CH%1").arg(msg.channel); // ECU
        }
    }
