...
AnalyzerPreviewWidget::AnalyzerPreviewWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::AnalyzerPreviewWidget)
{
    ui->setupUi(this);
...
AnalyzerPreviewWidget::~AnalyzerPreviewWidget()
{
    delete ui;
}
...
{
    struct Row { const char *time, *analyzer, *severity, *message; };
    ...
    for (int r = 0; r < 5; ++r) {
        ...
        sevLabel->setStyleSheet(
            "background-color: #451a1a; color: #f87171;"
...
void AnalyzerPreviewWidget::setupTimelineBar()
{
    ...
    bar->setStyleSheet(
        "background-color: #111218; border: 1px solid #272a35; border-radius: 4px;");
