...
NotebookView::NotebookView(TimelineWidget    *sharedTimeline,
...
void NotebookView::activate()
{
    ...
    if (m_timeline->parent() == this) return;
    ...
    for (auto *b : m_blocks) {
        if (b->contentWidget() == m_messageList) { msgBlockPresent = true; break; }
    }
    if (!msgBlockPresent) {
    ...
    for (auto *b : m_blocks) {
        if (b->contentWidget() == m_scriptEditor) { scriptBlockPresent = true; break; }
    }
...
NotebookBlockWidget *NotebookView::wrapInBlock(const QString &title, QWidget *content)
{
    ...
    connect(block, &NotebookBlockWidget::deleteRequested,
            this, &NotebookView::onBlockDeleteRequested);
    connect(block, &NotebookBlockWidget::dragStarted,
            this, &NotebookView::onBlockDragStarted);
    return block;
}
...
void NotebookView::addSignalViewBlock()
{
    ...
    for (auto *b : m_blocks) {
        if (b->contentWidget() == m_timeline) return;
    }
    m_timeline->setParent(nullptr);
    auto *block = wrapInBlock("Signal View", m_timeline);
    appendBlock(block);
...
void NotebookView::addScriptBlock()
{
    ...
    for (auto *b : m_blocks) {
        if (b->contentWidget() == m_scriptEditor) return;
    }
...
void NotebookView::addMarkdownBlock()
{
    auto *placeholder = new QLabel(
        "[ Markdown block — editor not yet implemented ]", m_canvas);
